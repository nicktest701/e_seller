import styles from "../../styles/Stadium.module.css";
import { currencyFormatter } from "../../constants/currencyFormatter";
function StadiumTemplateItem({ _id, voucher, info }) {
  return (
    <div className={styles.container}>
      <div class={styles.template}>
        <h1>Football TICKET</h1>
        <div class={styles.main}>
          <p>{info?.date}</p>
          <div>
            <span>Kick Off:</span>
            <span>{info?.time}</span>
          </div>

          <div class={styles.team}>
            <p>{info?.home} </p>
            <div>
              <svg
                version="1.0"
                xmlns="http://www.w3.org/2000/svg"
                width="70.000000pt"
                height="50.000000pt"
                viewBox="0 0 210.000000 200.000000"
              >
                <g
                  transform="translate(0.000000,200.000000) scale(0.100000,-0.100000)"
                  fill="#000000"
                  stroke="none"
                >
                  <path
                    d="M1319 1394 c-158 -191 -153 -186 -170 -179 -11 4 -38 -31 -104 -136
                   -49 -78 -90 -138 -91 -133 -1 5 -8 69 -15 142 l-12 133 -46 -3 c-25 -2 -45 -7
                   -44 -13 0 -5 11 -91 23 -190 12 -99 23 -185 25 -190 1 -6 20 -9 43 -7 36 3 43
                   9 73 55 153 242 413 619 421 612 2 -2 4 5 4 15 0 11 -1 20 -1 20 -1 0 -48 -57
                   -106 -126z m-357 -478 c7 4 8 3 4 -4 -4 -7 -1 -12 6 -12 7 -1 0 -7 -17 -14
                   -25 -11 -31 -11 -38 1 -5 8 -4 20 2 27 6 8 9 22 6 32 -4 11 1 6 10 -9 9 -16
                   21 -25 27 -21z"
                  />
                  <path
                    d="M1240 1162 c-42 -21 -80 -75 -80 -113 0 -35 39 -82 82 -99 41 -17 88
                   -49 88 -60 0 -5 -7 -16 -16 -24 -23 -24 -85 -20 -126 8 l-35 24 -141 -172
                   c-77 -94 -150 -181 -161 -193 -11 -12 -17 -24 -12 -27 5 -3 33 22 62 57 30 34
                   92 102 139 151 l86 88 54 -16 c70 -21 105 -20 157 4 47 21 83 70 83 112 0 38
                   -45 86 -107 113 -57 26 -73 46 -53 70 18 21 86 19 119 -4 l26 -19 34 30 c37
                   33 37 35 -34 68 -50 24 -119 25 -165 2z"
                  />
                </g>
              </svg>
            </div>
            <p>{info?.away}</p>
          </div>
          <p class={styles.stadium_name}>{info?.venue}</p>
          <div class={styles.details}>
            <div class={styles.stand}>
              <span>{voucher?.stand}</span>
              <span>{voucher?.price}</span>
            </div>
            <div class={styles.qrbox}>
              <img src={voucher?.qrCode} alt="code" />
            </div>

            <div class={styles.footballer}>
              <svg
                fill="#000000"
                version="1.1"
                id="Capa_1"
                width="50px"
                height="50px"
                viewBox="0 0 311.588 311.588"
              >
                <g>
                  <g>
                    <path
                      d="M181.731,0c15.202,0,27.526,12.323,27.526,27.526c0,15.2-12.324,27.524-27.526,27.524
			c-15.198,0-27.524-12.324-27.524-27.524C154.207,12.323,166.533,0,181.731,0z"
                    />
                    <path
                      d="M275.502,153.914c1.579,0.577,3.185,0.852,4.769,0.852c5.618,0,10.89-3.456,12.95-9.028
			c8.782-23.84-5.25-58.386-6.876-62.246l-1.942-4.59l-48.818-25.287c-2.89-1.497-6.054-1.815-9.016-1.225
			c-22.126-1.9-51.222,14.374-58.787,18.892c-1.196,0.714-1.869,1.139-1.869,1.139v0.031c-1.274,0.859-2.465,1.856-3.412,3.182
			l-26.502,37.037l-35.302,17.373c-6.835,3.366-9.652,11.633-6.289,18.468c2.403,4.878,7.296,7.71,12.386,7.71
			c2.048,0,4.119-0.455,6.082-1.421l41.523-20.436l13.729-19.18l2.284,44.1c0,0,0.149,0.057,0.408,0.16
			c0.062,0.777,0.156,1.559,0.363,2.341l4.442,17.124l-26.535,30.059l-66.238,80.033c-4.857,5.872-4.034,14.571,1.835,19.429
			c2.571,2.128,5.688,3.158,8.785,3.158c3.966,0,7.907-1.698,10.639-4.997l65.818-79.541l14.064-15.938l1.388,5.386l7.694,2.112
			c21.35,5.872,47.101,14.344,54.251,18.419c2.512,3.552,6.654,5.836,11.258,5.836c0.315,0,0.637-0.011,0.958-0.031
			c7.606-0.523,13.344-7.109,12.82-14.717c-0.787-11.516-11.309-16.984-40.55-26.615c-8.9-2.936-17.688-5.566-23.7-7.307
			l-2.086-8.036l9.206-10.434c8.891-5.593,24.984-52.962,28.045-87.073l19.351,10.028c3.713,10.545,8.104,28.283,4.701,37.507
			C264.689,143.346,268.355,151.278,275.502,153.914z"
                    />
                    <path
                      d="M44.475,246.033c16.003,0,28.977,12.976,28.977,28.977c0,16-12.974,28.977-28.977,28.977S15.498,291.01,15.498,275.01
			C15.498,259.009,28.472,246.033,44.475,246.033z"
                    />
                  </g>
                </g>
              </svg>
            </div>
          </div>
        </div>
      </div>
    </div>
  );
}

export default StadiumTemplateItem;
